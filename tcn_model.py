...
from tensorflow.keras.layers import LSTM, Dense, Conv1D, TimeDistributed, Flatten, Activation, Dropout, Bidirectional, GRU
from tensorflow.keras.layers import MaxPooling1D, Attention, Concatenate, GlobalAveragePooling1D, Embedding, concatenate, RepeatVector
from tensorflow.keras.callbacks import History, TensorBoard, Callback
import tensorflow.keras.initializers as KI
from tensorflow.keras.layers import BatchNormalization, Permute, multiply, AdditiveAttention
...
def create_model(num_classes, batch_size, train_data_shape, dropout_rate=0.3,
    ...
    :param stateful: whether to make the [B]LSTM(s) stateful; FIXME not used yet
    ...
    model.compile(optimizer=optimizer,loss='categorical_crossentropy',
                        metrics=['accuracy',f1_SP,f1_FIX,f1_SACC])
    model.summary()
    return model
...
def evaluate_test(args, model, X, y=None,
...
def get_feature_descriptor(args):
    ...
    naming_priority = ['movement', 'speed', 'acc', 'direction', 'standard_dev', 'displacement']
...
def get_arff_attributes_to_keep(args):
    ...
    if 'speed' in args.features:
        keys_to_keep += ['speed_{}'.format(i) for i in (1, 2, 4, 8, 16, 32, 64, 128)[:args.num_feature_scales]]
    if 'direction' in args.features:
        keys_to_keep += ['direction_{}'.format(i) for i in (1, 2, 4, 8, 16, 32, 64, 128)[:args.num_feature_scales]]
    if 'acc' in args.features:
        keys_to_keep += ['acceleration_{}'.format(i) for i in (1, 2, 4, 8, 16, 32, 64, 128)[:args.num_feature_scales]]
    if 'standard_dev' in args.features:
        keys_to_keep += ['standard_dev_{}'.format(i) for i in (1, 2, 4, 8, 16, 32, 64, 128)[:args.num_feature_scales]]
    if 'displacement' in args.features:
        keys_to_keep += ['displacement_{}'.format(i) for i in (1, 2, 4, 8, 16, 32, 64, 128)[:args.num_feature_scales]]
    return keys_to_keep
...
def run(args):
    ...
    for i, video_name in enumerate(all_video_names):
        ...
        if not ALREADY_TRAINED:
            ...
            if args.initial_epoch:
                ...
                # otherwise
                model = tf.keras.models.load_model(MODEL_PATH + '/Conv_sample_windows_epochs_{}_without_{}.h5'.format(args.initial_epoch, video_name),
                                                custom_objects={'f1_SP': f1_SP, 'f1_SACC': f1_SACC, 'f1_FIX': f1_FIX, 'TCN':TCN})
            ...
            model = tf.keras.models.load_model(model_fname, custom_objects={'f1_SP': f1_SP,
                                                                         'f1_SACC': f1_SACC,
                                                                         'f1_FIX': f1_FIX,
                                                                         'TCN': TCN})
        ...
        if TRAINING_MODE and not ALREADY_TRAINED:
            ...
            callbacks_list = [History(),
                              ...
                              tf.keras.callbacks.ModelCheckpoint(model_fname, monitor='val_loss', save_best_only=True, mode='min'),
                              TensorBoard(batch_size=args.batch_size, log_dir=log_root_dir.format(logname),
        ...
        raw, processed = evaluate_test(args, model, data_X[i], data_Y_one_hot[i],
        ...
        if args.final_run and args.output_folder is not None:
            ...
            for source_fname, source_obj, labels_true, labels_pred in \
                ...
                source_obj['data'][sp_processor.EM_TYPE_ATTRIBUTE_NAME] = \
                    list(map(lambda x: CORRESPONDENCE_TO_HAND_LABELLING_VALUES_REVERSE[x], labels_pred))
    ...
    if not ALREADY_TRAINED or args.final_run:
        ...
        if args.final_run and args.output_folder:
            ...
            cmd = 'python3 {sp_tool_dir}/examples/run_evaluation.py '\
                  '--in "{out_dir}" '\
                  '--hand "{gt_dir}" '\
                  '> '\
...
def parse_args(dry_run=False):
    ...
    parser.add_argument('--model-folder', dest='model_folder', default='data/new_models/')
    ...
    parser.add_argument('--features', '--feat', choices=['movement',  # = speed + direction + acceleration
                                                         'speed', 'acc', 'direction', 'standard_dev', 'displacement',
                                                         'xy'],
    ...
    if not 1 <= args.num_feature_scales <= 8:
        raise ValueError('--num-feature-scales can be between 1 and 8')
