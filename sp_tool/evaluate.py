...
def evaluate_normalised_Levenshtein_dist(true_labels_list,
    ...
    for ground_truth, assigned_labels in zip(true_labels_list, assigned_labels_list):
        ...
        # print("GT_LABELS:", ground_truth_labels)
        # print("ASSIGNED:", assigned_labels['data']['EYE_MOVEMENT_TYPE'])
        # input()
        ...
        if verbose:
            print ('For the positive label of {}, using the following mapping: {}'.format(positive_label, all_unique_labels_mapping))
...
def evaluate_episodes_as_Zemblys_et_al(true_labels_list,
    ...
    if positive_label is not None:
        true_labels_events = list(map(lambda x: x if x == positive_label else '_WRONG_LABEL', true_labels_events))
        assigned_labels_events = list(map(lambda x: x if x == positive_label else '_WRONG_LABEL', assigned_labels_events))
...
def evaluate_episodes_as_Hooge_et_al(true_labels_list,
    ...
    for ground_truth, assigned_labels in zip(true_labels_list, assigned_labels_list):
        ...
        ground_truth_events = list(filter(lambda x: x.type == positive_label, ground_truth_events))
        assigned_events = list(filter(lambda x: x.type == positive_label, assigned_events))
    ...
    for ground_truth, assigned_labels in zip(true_labels_list, assigned_labels_list):
        ...
        if positive_label is not None:
            ...
            if not ground_truth_labels.dtype.name.startswith('str'):
                ground_truth_labels = list(map(lambda x: EM_VALUE_MAPPING_DEFAULT[x], ground_truth_labels))
